...
def sources():
    ...
    s["d_ben"] = e[e.tier == 2]["delta_MRR@10"].mean()
    s["d_harm"] = e[e.tier == 1]["delta_MRR@10"].mean()
    ...
    s["boot"] = b
    s["pooled"] = b.groupby(["Nominal tier", "Documents sampled"])[
...
def check_values(t, s):
    check("d_ben +0.0627 in text", "0.0627" in t or "0.062686" in t,
          f"source {s['d_ben']:+.6f}")
    check("d_harm 0.0867 in text", "0.086747" in t or "0.0867" in t,
    ...
    m = re.search(r"exact threshold \((0\.\d+)\)", t)
...
def check_internal(t, s):
    ...
    check("31/55 below p*", 31 / 55 < s["p_zero"],
          f"{31/55:.4f} vs {s['p_zero']:.4f}")
    ...
    ths = sorted(set(re.findall(r"0\.5805\d*", t)))
