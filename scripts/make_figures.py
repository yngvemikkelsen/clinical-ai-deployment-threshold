...
Output: 600 dpi PNG and vector PDF, greyscale-safe, no reliance on colour alone.
"""
...
def figure1():
    ...
    arrow(19.0, 18.58)
    # The threshold line is set one point smaller and split across two lines so
    # it stays inside the box at the figure's width.
    ...
    arrow(14.38, 13.92, x=3.4)
    arrow(14.38, 13.92, x=8.6)
    box(12.3, "A. Cross-condition\nevidence breadth\n\n"
              "corpus × query-format\nconditions scored",
        x=3.4, w=3.6, h=3.1, fs=8.0)
    box(12.3, "B. Within-site\ndocument sample\n\n"
              "documents in joint\nfit/evaluation sample\nper condition",
        x=8.6, w=3.6, h=3.1, fs=8.0)
    ax.text(CX, 12.3, "both\nrequired", ha="center", va="center",
            fontsize=7.6, style="italic", color=MID, linespacing=1.4)
    arrow(10.68, 10.2, x=3.4)
    arrow(10.68, 10.2, x=8.6)
...
def figure2():
    ...
    # se/sp and the requirement are DERIVED, not hardcoded: import the
    # canonical model so the figure cannot drift from the manuscript.
    import importlib.util
    _s = importlib.util.spec_from_file_location("model",
                                                "clinical_ai_deployment_model.py")
    hm = importlib.util.module_from_spec(_s); _s.loader.exec_module(hm)
    _eff = hm.load_effects()
    _, _, DBEN, DHARM = hm.response_groups(_eff[hm.BASE_EPSILON])
    H, M_BASE = abs(DHARM), hm.value_exposed()
    T_COST = hm.T_DEPLOY
    t_ = T_COST / M_BASE
    rows = []
    for nc in range(1, 7):
        se_, sp_ = hm.derive_screen(nc, _eff[hm.BASE_EPSILON])
        s_ = (400 * nc) / M_BASE
        B_ = (1 - sp_) * (H + t_)
        rows.append({"n_cond": nc, "se": se_, "sp": sp_,
                     "p_screen_star": (B_ + s_) / (se_ * (DBEN - t_) + B_)})
    heldout = pd.DataFrame(rows)
    DB, DH = 0.075745, -0.076593
    # External curve: the SAME exact S/T equation as the benchmark curve, but
    # with the institutional effect magnitudes. Using the benchmark harm
    # magnitude here (as an earlier version did) puts the dashed curve well
    # below the values reported in Table 5.
    # Derived from the transport matrix rather than hardcoded, so the figure
    # cannot drift from Table 5. Definition follows the manuscript: benefit
    # and harm magnitudes are the means of the positive and negative
    # section-level ("hpi") institutional cells. The literals are a documented
    # fallback only and reproduce Table 5 to within 1e-4.
    _tm = pd.read_csv("appendices/appendix2_transport_matrix.csv")
    _hpi = _tm[_tm["Document variant"] == "hpi"]["dMRR@10"].astype(float)
    DBEN_EXT = float(_hpi[_hpi > 0].mean())
    H_EXT = float(abs(_hpi[_hpi < 0].mean()))
    # The figure must not silently disagree with the manuscript. These are the
    # magnitudes reported in Results and Table 5; a mismatch means the matrix
    # or the manuscript has changed and both must be reconciled before the
    # figure is regenerated.
    assert abs(DBEN_EXT - 0.142) < 5e-4 and abs(H_EXT - 0.101) < 5e-4, (
        f"institutional magnitudes {DBEN_EXT:+.4f}/{-H_EXT:.4f} disagree with "
        "the +0.142/-0.101 reported in Results and Table 5")
    print(f"  external effect magnitudes: +{DBEN_EXT:.4f} / -{H_EXT:.4f}")
    ext = {}
    for r in heldout.itertuples():
        s_ = (400 * int(r.n_cond)) / M_BASE
        B_ = (1 - r.sp) * (H_EXT + t_)
        ext[int(r.n_cond)] = (B_ + s_) / (r.se * (DBEN_EXT - t_) + B_)
    ...
    a.set_ylabel("minimum prior probability of benefit")
    ...
    b.set_xlabel("documents in joint fit/evaluation sample")
