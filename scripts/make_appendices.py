...
derived here from the primary artefact; nothing is transcribed by hand.
  ...
  5  Decision-model parameters, epsilon sweep, economic sensitivity outputs
...
U = Path(os.environ.get("PAPER9_DATA", HERE))
OUT = Path("appendices")
...
def appendix1():
    ...
    confirmed affected (1 of 55). This appendix is the audit trail.
    """
    ...
    # the coding rule, stated rather than left implicit: 1/55 is load-bearing
...
def appendix5():
    ...
    P = [
        ...
        ("se", "Screen sensitivity", "derived", "derived separately by scored-condition breadth and by document-resampling design",
         "proportion", "Between-condition variability; resampling, Appendix 3",
         "empirical", "technical effect"),
        ("sp", "Screen specificity", "derived", "derived separately by scored-condition breadth and by document-resampling design",
         "proportion", "Between-condition variability; resampling, Appendix 3",
         "empirical", "technical effect"),
        ...
        ("C_event", "Treatment cost per adverse event", "EUR 11,494",
         "Gamma(2.5, 4597)", "EUR",
         "SAMDATA 2024 bed-day NOK 26,153 x 5.11 excess days (Hoogervorst-Schilp 2015) "
         "= NOK 133,642, converted at FX. Exceeds the European estimates of Durand 2024 "
         "(EUR 6,000-10,000 per hospitalisation) and Laroche 2025 (EUR 5,974 mean, "
         "EUR 618-27,380, 2023 tariffs); both are carried in the cost sensitivity "
         "analysis, across which the threshold is unchanged at 0.50",
        ...
        ("lambda", "Willingness to pay", "NOK 275,000",
         ...
         "Meld. St. 34 (2015-2016). Applied at nominal policy value without "
         "indexation; the Norwegian Directorate for Medical Products reviewed the "
         "thresholds in 2026 and recommended no interim adjustment",
         "empirical", "enters M via V"),
        ("FX", "Exchange rate", "11.6276", "fixed", "NOK/EUR",
         "Norges Bank annual average for 2024, the price year of the SAMDATA "
         "bed-day estimate; accessed September 19, 2026",
         "empirical", "enters M via V"),
        ...
        ("S", "Fixed screening cost", "EUR 400 x n_cond", "= K_screen", "EUR",
         "Incurred whichever way the screen reads; EUR 1,600 at n_cond = 4",
         "scenario", "enters the screening threshold as s = S/M"),
        ("T", "Deployment cost", "EUR 1,468",
         "K_impl + K_annual x D", "EUR",
         "Incurred only on the systems the screen selects, at a rate that "
         "depends on the site prior probability of benefit. S and T are carried "
         "separately rather than combined into a single screen-and-treat cost, "
         "because combining them would require assuming a value for p",
         "scenario", "enters the screening threshold as t = T/M"),
